-- Corrected Organization Logo Setup Script
...
-- 9. Create index on logo_url for better performance
CREATE INDEX IF NOT EXISTS idx_organizations_logo_url ON public.organizations(logo_url);

-- 10. Add validation constraint to ensure logo_url is a valid URL format
ALTER TABLE public.organizations 
ADD CONSTRAINT IF NOT EXISTS check_logo_url_format 
CHECK (logo_url IS NULL OR logo_url ~ '^https?://.*');

-- 11. Create function to get organization logo URL
...
-- 13. Create function to update organization logo
...
-- 14. Grant execute permission on the update function
GRANT EXECUTE ON FUNCTION public.update_organization_logo(UUID, TEXT) TO authenticated;

-- 15. Create audit log entry for logo changes (using correct column names)
...
BEGIN
  INSERT INTO public.audit_logs (
    ...
    auth.uid(),
    format('Organization logo %s', action),
    'organizations',
    org_id,
    CASE WHEN old_logo_url IS NOT NULL THEN jsonb_build_object('logo_url', old_logo_url) ELSE NULL END,
    CASE WHEN new_logo_url IS NOT NULL THEN jsonb_build_object('logo_url', new_logo_url) ELSE NULL END,
    NOW()
  );
END;
$$;

-- 16. Grant execute permission on the audit function
GRANT EXECUTE ON FUNCTION public.log_logo_change(UUID, TEXT, TEXT, TEXT) TO authenticated;

-- 17. Create trigger to log logo changes
...
-- 20. Create a view for organizations with logo information
...
-- 21. Grant select permission on the view
GRANT SELECT ON public.organizations_with_logos TO authenticated;

-- 22. Final verification query
