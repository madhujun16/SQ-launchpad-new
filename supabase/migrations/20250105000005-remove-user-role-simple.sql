-- Simple migration to remove 'user' role from app_role enum
-- Since 'user' role doesn't exist, we skip the deletion step

-- Step 1: Create a new enum without 'user' role
CREATE TYPE public.app_role_new AS ENUM ('admin', 'ops_manager', 'deployment_engineer');

-- Step 2: Temporarily disable RLS on user_roles to avoid policy conflicts
ALTER TABLE public.user_roles DISABLE ROW LEVEL SECURITY;

-- Step 3: Update existing records with explicit casting
...
-- Step 4: Now alter the column type
