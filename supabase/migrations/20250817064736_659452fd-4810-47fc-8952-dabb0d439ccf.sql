...
-- Create audit trigger for license access
CREATE OR REPLACE FUNCTION public.audit_license_access()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
  -- Log all access to sensitive license data
  INSERT INTO public.audit_logs (
    ...
    'Accessed license: ' || NEW.id,
    jsonb_build_object(
      'license_id', NEW.id,
      'license_name', NEW.name,
      'access_time', now(),
      'user_role', (SELECT array_agg(role) FROM public.user_roles WHERE user_id = auth.uid())
    )
  );
...
DROP TRIGGER IF EXISTS audit_license_sensitive_access ON public.licenses;
CREATE TRIGGER audit_license_sensitive_access
  AFTER SELECT ON public.licenses
  FOR EACH ROW
  WHEN (NEW.license_key IS NOT NULL OR NEW.cost IS NOT NULL)
  EXECUTE FUNCTION public.audit_license_access();