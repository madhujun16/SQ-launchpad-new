-- Complete migration to remove 'user' role from app_role enum
-- Handles all RLS policy dependencies
...
-- Step 3: Temporarily disable ALL RLS policies that depend on role column
-- Disable RLS on user_roles
ALTER TABLE public.user_roles DISABLE ROW LEVEL SECURITY;

-- Disable RLS on licenses (which has policies that depend on role)
ALTER TABLE public.licenses DISABLE ROW LEVEL SECURITY;

-- Disable RLS on other tables that might have role-dependent policies
ALTER TABLE public.profiles DISABLE ROW LEVEL SECURITY;
ALTER TABLE public.sites DISABLE ROW LEVEL SECURITY;
ALTER TABLE public.inventory_items DISABLE ROW LEVEL SECURITY;
ALTER TABLE public.inventory_deployment_history DISABLE ROW LEVEL SECURITY;
ALTER TABLE public.inventory_maintenance_log DISABLE ROW LEVEL SECURITY;
