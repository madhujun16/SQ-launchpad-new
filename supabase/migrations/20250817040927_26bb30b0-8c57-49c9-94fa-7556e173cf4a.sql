-- Create Notifications System (Fixed with IF NOT EXISTS)
...
-- 5. Drop existing policies first to avoid conflicts
...
DROP POLICY IF EXISTS "notifications: admin can view all" ON public.notifications;
DROP POLICY IF EXISTS "notifications: system can create" ON public.notifications;
DROP POLICY IF EXISTS "notification_preferences: user can manage own" ON public.notification_preferences;
...
-- 9. Create function to get notifications with role-based filtering
